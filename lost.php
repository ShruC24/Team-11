					...
					<form action="" id="items-form">
						...
						<div class="row">
							<div class="form-group col-lg-12 col-md-12 col-sm-12 col-xs-12">
								...
								<input type="text" name="title" id="title" class="form-control form-control-sm rounded-0" value="<?php echo isset($title) ? $title : ''; ?>"  required maxlength="30"/>
							</div>
						</div>
						<div class="row">
							<div class="form-group col-lg-12 col-md-12 col-sm-12 col-xs-12">
...
<script>
	...
	$(document).ready(function(){
		...
		$('#items-form').submit(function(e){
			...
			var titleMaxLength_contact = 10; // Set the desired maximum length for the title

			if ($('#title').val().length > titleMaxLength_contact) {
				alert_toast('Title exceeds the maximum length of ' + titleMaxLength_contact + ' characters.', 'error');
				return;
			}
            var _this = $(this)
			 $('.err-msg').remove();
